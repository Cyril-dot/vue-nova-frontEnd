  ...
  <div class="nv-root">
    ...
    <div v-if="view === 'meeting'" class="nv-meet">
      ...
      <div class="nv-jitsi-wrap">
        <div v-if="loading" class="nv-daily-loading">
...
export default {
  ...
  data() {
    return {
      ...
      meetingCode:      '',
      jitsiRoomUrl:     '',
      jitsiApi:         null,   // JitsiMeetExternalAPI instance
      loading:          true,
  ...
  methods: {
    ...
    async initMeeting() {
      ...
      // Fetch token + room URL from backend
      ...
      if (!roomUrl) {
        // Fallback: construct URL using meet.jit.si directly
        roomUrl = `https://meet.jit.si/${this.meetingCode}`;
        log('initMeeting() — using fallback URL:', roomUrl);
      }

      this.jitsiRoomUrl = roomUrl;

      this.loadingStatus = 'Loading Jitsi SDK…';
      try {
    ...
    loadJitsiSDK() {
      return new Promise((resolve, reject) => {
        ...
        s.onerror = (e) => { err('loadJitsiSDK() error:', e); reject(new Error('Failed to load Jitsi SDK')); };
    ...
    launchJitsi(token = null) {
      try {
        ...
        // Parse domain and room from URL or use defaults
        ...
        // Add JWT token if present (for JaaS)
    ...
    onJitsiError(event) {
      ...
      this.showToast('Connection error: ' + (event?.error?.message || 'Unknown'), 'error');
    },
    ...
    syncParticipantCount() {
      ...
      try {
        ...
        log('syncParticipantCount() error:', e.message);
      }
    },

    // ── Leave / End ───────────────────────────────────────────────────────────
    leave() {
      if (!confirm(this.isHost ? 'Leave? Use your browser back button or end for everyone.' : 'Leave this meeting?')) return;
      if (this.jitsiApi) {
...
.nv-root {
  ...
  --c-red-bg:rgba(234,67,53,.15);--c-orange:#fa7b17;--c-text:#e8eaed;--c-text2:#9aa0a6;--c-r:8px;
...
.nv-code-chip{display:flex;align-items:center;gap:6px;padding:5px 12px;background:var(--c-surf);border:1px solid var(--c-line);border-radius:var(--c-r);color:#8ab4f8;font-family:'Google Sans Mono',monospace;font-size:12px;cursor:pointer}
