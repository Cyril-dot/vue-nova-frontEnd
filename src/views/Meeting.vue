      ...
      <div class="nm-create-body">
        <div class="nm-create-left">
          ...
          <div class="nm-hero-features">
            <div class="nm-feat"><div class="nm-feat-dot"></div>HD video & crystal-clear audio</div>
            <div class="nm-feat"><div class="nm-feat-dot"></div>Screen sharing & collaboration</div>
            <div class="nm-feat"><div class="nm-feat-dot"></div>Secure JWT-authenticated rooms</div>
          </div>
        </div>
          ...
          <div class="nm-card">
            ...
            <div v-if="!created.code" class="nm-form">
              ...
              <label class="nm-toggle-row">
                ...
                <div class="nm-toggle"><div class="nm-toggle-knob"></div></div>
              ...
              <div v-if="createError" class="nm-error-inline">
    ...
    <div v-if="view === 'meeting'" class="nm-meeting">
      ...
      <header class="nm-meeting-bar">
        ...
        <div class="nm-bar-center">
          ...
          <div class="nm-bar-sep"></div>
          <button class="nm-code-chip" @click="copyMeetingCode" title="Click to copy meeting code">
          ...
          <div class="nm-participants-chip">
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M17 21v-2a4 4 0 00-4-4H5a4 4 0 00-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 00-3-3.87M16 3.13a4 4 0 010 7.75"/></svg>
            {{ participantCount }}
          </div>
        </div>
        ...
        <div class="nm-bar-right">
          ...
          <button class="nm-bar-btn nm-bar-btn--leave" @click="promptLeave">
      ...
      <transition name="nm-modal">
        <div v-if="showLeaveModal" class="nm-modal-overlay" @click.self="showLeaveModal = false">
          <div class="nm-modal">
            <div class="nm-modal-icon nm-modal-icon--orange">
              <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round">
                <path d="M9 21H5a2 2 0 01-2-2V5a2 2 0 012-2h4M16 17l5-5-5-5M21 12H9"/>
              </svg>
            </div>
            <h2 class="nm-modal-title">Leave this meeting?</h2>
            <p class="nm-modal-body">The meeting will continue for other participants. You can rejoin anytime using the meeting code.</p>
            <div class="nm-modal-code-preview">
            ...
            <div class="nm-modal-actions">
              <button class="nm-btn-ghost" @click="showLeaveModal = false">Stay in meeting</button>
              <button class="nm-btn-leave-confirm" @click="confirmLeave">
                <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><path d="M9 21H5a2 2 0 01-2-2V5a2 2 0 012-2h4M16 17l5-5-5-5M21 12H9"/></svg>
                Leave meeting
              </button>
            </div>
          </div>
      ...
      <transition name="nm-modal">
        <div v-if="showEndModal" class="nm-modal-overlay" @click.self="showEndModal = false">
          <div class="nm-modal">
            ...
            <p class="nm-modal-body">All participants will be immediately disconnected and the room will be permanently removed.</p>
            <div class="nm-modal-warning">
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/><line x1="12" y1="9" x2="12" y2="13"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>
              This action cannot be undone
            </div>
            <div class="nm-modal-actions">
              <button class="nm-btn-ghost" @click="showEndModal = false" :disabled="ending">Stay in meeting</button>
...
export default {
  ...
  methods: {
    ...
    onConferenceJoined() { this.loading = false; this.joinError = ''; this.syncParticipantCount(); },
    onConferenceLeft() { this.cleanupAndNavigate(); },
    ...
    // ── Leave / End flows ───────────────────────────────────────────────
    promptLeave() { this.showLeaveModal = true; },
...
/* ── CSS Variables — matches meetingDashboard.vue exactly ── */
.nm-shell {
  --blue:      #4a90e2;
  --blue-dk:   #2c6fbd;
  --blue-soft: #e8f2fc;
...
/* ── Logo ── */
.nm-logo { width: 34px; height: 34px; border-radius: 9px; background: linear-gradient(135deg, var(--blue), #6366f1); display: flex; align-items: center; justify-content: center; box-shadow: 0 4px 12px var(--blue-glow); flex-shrink: 0; }
...
.nm-topbar { height: 64px; display: flex; align-items: center; justify-content: space-between; padding: 0 32px; background: var(--white); border-bottom: 1px solid var(--border); position: sticky; top: 0; z-index: 50; box-shadow: var(--shadow-sm); }
.nm-brand { display: flex; align-items: center; gap: 10px; cursor: pointer; }
.nm-back-btn { display: flex; align-items: center; gap: 7px; padding: 8px 16px; background: var(--bg); border: 1px solid var(--border); border-radius: 8px; color: var(--ink-m); font-family: var(--font); font-size: 13px; font-weight: 500; cursor: pointer; transition: all .15s; }
.nm-back-btn:hover { border-color: var(--blue); color: var(--blue); background: var(--blue-soft); }

.nm-create-body { flex: 1; display: grid; grid-template-columns: 1fr 1fr; max-width: 1100px; margin: 0 auto; padding: 64px 32px; gap: 80px; align-items: center; width: 100%; }

.nm-hero-eyebrow { font-family: var(--fdisp); font-size: 11px; font-weight: 700; letter-spacing: 2px; color: var(--blue); background: var(--blue-soft); border: 1px solid var(--blue-mid); display: inline-block; padding: 5px 12px; border-radius: 20px; margin-bottom: 20px; }
.nm-hero-title { font-family: var(--fdisp); font-size: clamp(32px,4vw,48px); font-weight: 700; color: var(--ink); line-height: 1.12; margin-bottom: 16px; }
...
.nm-card { background: var(--white); border: 1px solid var(--border); border-radius: 16px; padding: 32px; box-shadow: var(--shadow-lg); }
.nm-card-header { display: flex; align-items: center; gap: 14px; margin-bottom: 28px; }
.nm-card-icon { width: 44px; height: 44px; border-radius: 10px; background: var(--blue-soft); border: 1px solid var(--blue-mid); display: flex; align-items: center; justify-content: center; color: var(--blue); flex-shrink: 0; }
...
.nm-input { width: 100%; padding: 12px 12px 12px 38px; background: var(--bg); border: 1.5px solid var(--border); border-radius: var(--r); color: var(--ink); font-family: var(--font); font-size: 14px; font-weight: 500; transition: all .15s; }
...
.nm-toggle { width: 40px; height: 22px; border-radius: 11px; background: var(--border); border: 1.5px solid #cbd5e0; position: relative; flex-shrink: 0; transition: all .2s; }
.nm-toggle-knob { width: 16px; height: 16px; border-radius: 50%; background: var(--white); position: absolute; top: 1px; left: 1px; transition: transform .2s; box-shadow: 0 1px 3px rgba(0,0,0,.15); }
...
/* Buttons */
.nm-btn-primary { display: inline-flex; align-items: center; justify-content: center; gap: 8px; padding: 12px 24px; background: var(--blue); color: var(--white); border: none; border-radius: var(--r); font-family: var(--font); font-size: 14px; font-weight: 700; cursor: pointer; transition: all .15s; box-shadow: 0 4px 14px var(--blue-glow); }
.nm-btn-primary:hover:not(:disabled) { background: var(--blue-dk); transform: translateY(-1px); }
.nm-btn-primary:disabled { opacity: .5; cursor: not-allowed; }
.nm-btn-full { width: 100%; }
.nm-btn-ghost { display: inline-flex; align-items: center; gap: 7px; padding: 10px 18px; background: transparent; border: 1.5px solid var(--border); border-radius: var(--r); color: var(--ink-m); font-family: var(--font); font-size: 13px; font-weight: 600; cursor: pointer; transition: all .15s; }
.nm-btn-ghost:hover:not(:disabled) { border-color: var(--blue); color: var(--blue); background: var(--blue-soft); }
.nm-btn-ghost:disabled { opacity: .55; cursor: not-allowed; }
.nm-btn-danger { display: inline-flex; align-items: center; gap: 8px; padding: 11px 22px; background: var(--red); color: var(--white); border: none; border-radius: var(--r); font-family: var(--font); font-size: 14px; font-weight: 700; cursor: pointer; transition: all .15s; }
.nm-btn-danger:hover:not(:disabled) { background: #dc2626; }
.nm-btn-danger:disabled { opacity: .55; cursor: not-allowed; }

/* Leave confirm button — orange tone */
.nm-btn-leave-confirm { display: inline-flex; align-items: center; gap: 8px; padding: 11px 22px; background: var(--orange); color: var(--white); border: none; border-radius: var(--r); font-family: var(--font); font-size: 14px; font-weight: 700; cursor: pointer; transition: all .15s; }
.nm-btn-leave-confirm:hover { background: #d97706; }

.nm-error-inline { display: flex; align-items: center; gap: 8px; padding: 11px 14px; background: #fef2f2; border: 1px solid #fecaca; border-radius: var(--r); font-size: 13px; color: #dc2626; }

.nm-spinner { display: inline-block; width: 14px; height: 14px; border: 2px solid rgba(255,255,255,.3); border-top-color: #fff; border-radius: 50%; animation: nm-spin .6s linear infinite; }
@keyframes nm-spin { to { transform: rotate(360deg); } }

.nm-success { display: flex; flex-direction: column; gap: 20px; }
.nm-success-check { width: 52px; height: 52px; border-radius: 50%; background: #ecfdf5; border: 1px solid #6ee7b7; display: flex; align-items: center; justify-content: center; color: var(--green); }
...
.nm-code { font-family: 'Courier New', monospace; font-size: 18px; font-weight: 700; color: var(--blue); background: var(--blue-soft); border: 1px solid var(--blue-mid); padding: 6px 14px; border-radius: 7px; letter-spacing: 1px; }
.nm-copy-btn { display: flex; align-items: center; gap: 5px; padding: 7px 12px; background: var(--white); border: 1.5px solid var(--border); border-radius: 7px; color: var(--ink-m); font-family: var(--font); font-size: 12px; font-weight: 600; cursor: pointer; transition: all .15s; }
...
/* ══════════════════════════════
   MEETING VIEW — Google Meet + Zoom hybrid
══════════════════════════════ */
.nm-meeting { display: flex; flex-direction: column; position: fixed; inset: 0; z-index: 9999; background: #111827; }

/* Top bar — clean white like Google Meet */
.nm-meeting-bar {
...
.nm-bar-left { display: flex; align-items: center; gap: 10px; flex: 1; min-width: 0; }
.nm-bar-right { display: flex; align-items: center; gap: 8px; flex: 1; justify-content: flex-end; }
.nm-bar-center { display: flex; align-items: center; gap: 10px; flex-shrink: 0; }
.nm-bar-brand { display: flex; align-items: center; gap: 8px; flex-shrink: 0; }
.nm-bar-name { font-family: var(--fdisp); font-size: 14px; font-weight: 700; color: var(--ink); }
.nm-bar-sep { width: 1px; height: 20px; background: var(--border); flex-shrink: 0; }
.nm-bar-meta { display: flex; align-items: center; gap: 6px; }

.nm-code-chip { display: flex; align-items: center; gap: 6px; padding: 6px 12px; background: var(--blue-soft); border: 1.5px solid var(--blue-mid); border-radius: 8px; font-family: 'Courier New', monospace; font-size: 12px; font-weight: 700; color: var(--blue); cursor: pointer; transition: all .15s; white-space: nowrap; }
.nm-code-chip:hover { background: var(--blue-mid); }

.nm-participants-chip { display: flex; align-items: center; gap: 5px; padding: 5px 10px; background: var(--bg); border: 1px solid var(--border); border-radius: 8px; font-size: 13px; color: var(--ink-m); font-weight: 600; }

.nm-live-badge { display: flex; align-items: center; gap: 6px; padding: 4px 10px; border-radius: 20px; background: #fef2f2; border: 1px solid #fecaca; font-size: 9px; font-weight: 800; letter-spacing: 1px; color: #dc2626; }
...
.nm-host-badge { padding: 4px 10px; border-radius: 20px; background: var(--blue-soft); border: 1px solid var(--blue-mid); font-size: 9px; font-weight: 800; letter-spacing: 1px; color: var(--blue); }
.nm-clock { font-size: 13px; color: var(--ink-m); font-weight: 600; }

/* Action buttons — Zoom style pill buttons */
.nm-bar-btn { display: flex; align-items: center; gap: 6px; padding: 8px 16px; border-radius: 8px; font-family: var(--font); font-size: 12px; font-weight: 700; cursor: pointer; border: 1.5px solid; transition: all .15s; }
...
/* Leave button — red, prominent like Zoom */
.nm-bar-btn--leave { background: var(--red); border-color: var(--red); color: var(--white); box-shadow: 0 2px 8px rgba(239,68,68,0.3); }
.nm-bar-btn--leave:hover { background: #dc2626; border-color: #dc2626; transform: translateY(-1px); }

/* Jitsi frame */
.nm-frame-wrap { flex: 1; position: relative; background: #111827; overflow: hidden; }
.nm-jitsi { width: 100%; height: 100%; }
.nm-jitsi iframe { width: 100% !important; height: 100% !important; border: none !important; }

/* Loading */
.nm-loading { position: absolute; inset: 0; z-index: 10; background: rgba(17,24,39,0.95); display: flex; align-items: center; justify-content: center; }
.nm-loader-card { display: flex; flex-direction: column; align-items: center; gap: 20px; padding: 48px 48px 40px; background: var(--white); border: 1px solid var(--border); border-radius: 20px; min-width: 260px; box-shadow: var(--shadow-lg); }
.nm-loader-ring { width: 52px; height: 52px; border-radius: 50%; border: 3px solid var(--blue-mid); border-top-color: var(--blue); animation: nm-spin .8s linear infinite; }
...
/* Error overlay */
.nm-error-overlay { position: absolute; inset: 0; z-index: 10; background: rgba(17,24,39,0.95); display: flex; align-items: center; justify-content: center; }
.nm-error-card { display: flex; flex-direction: column; align-items: center; gap: 14px; padding: 48px 40px; text-align: center; background: var(--white); border: 1px solid #fecaca; border-radius: 20px; max-width: 360px; box-shadow: var(--shadow-lg); }
...
/* ── Modals — proper overlay popups ── */
.nm-modal-overlay { position: fixed; inset: 0; z-index: 20000; background: rgba(13,27,54,.55); backdrop-filter: blur(8px); display: flex; align-items: center; justify-content: center; padding: 20px; }
.nm-modal { background: var(--white); border: 1px solid var(--border); border-radius: 20px; padding: 40px 36px 32px; max-width: 440px; width: 100%; text-align: center; box-shadow: 0 24px 64px rgba(0,0,0,0.18); }

.nm-modal-icon { width: 64px; height: 64px; border-radius: 50%; display: flex; align-items: center; justify-content: center; margin: 0 auto 20px; }
.nm-modal-icon--red    { background: #fef2f2; border: 2px solid #fecaca; color: #dc2626; }
.nm-modal-icon--orange { background: #fffbeb; border: 2px solid #fde68a; color: #d97706; }

.nm-modal-title { font-family: var(--fdisp); font-size: 22px; font-weight: 700; color: var(--ink); margin-bottom: 10px; }
.nm-modal-body  { font-size: 14px; color: var(--ink-m); line-height: 1.7; margin-bottom: 16px; }

/* Meeting code preview inside modal */
.nm-modal-code-preview { display: inline-flex; align-items: center; gap: 7px; padding: 8px 16px; background: var(--blue-soft); border: 1.5px solid var(--blue-mid); border-radius: 8px; margin-bottom: 24px; color: var(--blue); font-size: 13px; }
.nm-modal-code-preview code { font-family: 'Courier New', monospace; font-weight: 700; letter-spacing: .5px; }

/* Warning strip */
.nm-modal-warning { display: flex; align-items: center; justify-content: center; gap: 7px; padding: 10px 14px; background: #fffbeb; border: 1px solid #fde68a; border-radius: 8px; font-size: 13px; font-weight: 600; color: #92400e; margin-bottom: 24px; }

.nm-modal-actions { display: flex; gap: 10px; justify-content: center; margin-top: 0; }

/* Toast */
.nm-toast { position: fixed; bottom: 24px; right: 24px; z-index: 30000; display: flex; align-items: center; gap: 8px; padding: 11px 18px; border-radius: 10px; font-size: 13px; font-weight: 700; box-shadow: var(--shadow-lg); pointer-events: none; }
...
@media (max-width: 768px) {
  ...
  .nm-bar-name { display: none; }
  .nm-clock { display: none; }
}
</style>